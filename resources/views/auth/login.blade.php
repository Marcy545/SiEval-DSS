...
<div class="w-full max-w-3xl bg-white rounded-[32px] shadow-2xl px-14 py-12 relative z-20">
    ...
    {{-- FIX: Menambahkan parameter query string agar rute login mengenali role saat disubmit --}}
    <form action="{{ route('login', ['role' => $role]) }}" method="POST" class="space-y-6 mt-8">
        @csrf
        
        {{-- Input hidden tambahan untuk menjaga persistensi role --}}
